...
`#[UseStrategy]` names a class whose public static `create()` builds a `CacheStrategy` — typed arguments in, composed strategy out, resolved once per memoized declaration and part of the fingerprint. Composing strategies with `CompositeCacheStrategy::compose()` yields another `CacheStrategy`, so compositions compose again, and the order decides pre/post-processing and failure capture ranges. The runtime executes every stage through the chain — the declared composition in front of the internal `TerminalCacheStrategy`, which owns the stage bodies and answers instead of delegating — so a strategy wraps the fixed stages but cannot reorder them. Strategy constraints are met before the policy constraint at the single base time, which lets `Ttl::Auto` derive from what the strategies guarantee; everything still goes through the metadata meet, so no strategy can extend an expiration a dependency imposed. A strategy publishes the effect of its normal origin path as `Strategy\Contract` attributes (`#[Ttl(min: new ConstructorArg('minimum'))]` on `fetch()`, `unconstrained: true` for "adds nothing"); a composition never re-declares child contracts, and `#[AssumeTtl]` on `create()` is an analysis-only assumption that replaces exactly the one item it names.
...
The two failure-handling behaviors follow the same split instead of escaping it. `#[StaleIfError]` only accepts declared behavior: every declared exception type must be a `RuntimeException` subtype, so `CacheRuntime` catches exactly `RuntimeException` around the origin call, serves a retained stale entry only on a declared match, and rethrows the original otherwise — a bug (`LogicException` family, `Error`) never enters the catch and can never be answered with stale data. An origin that meets an expected outage in a foreign hierarchy — a PSR-18 client, `Doctrine\DBAL\Exception`, `JsonException` — translates it into its own declared `RuntimeException` subtype at the boundary, which is the same translation duty the bundled adapters already perform for storage. Likewise `GuardedCache` catches exactly `RuntimeException` around cache reads and writes, because `CacheBackendFailure` is what the `Cache` port declares; the `#[BypassCacheErrors]` classifier decides within that family, and a backend that reports outside it violates the port contract and propagates as the bug it is. Neither boundary needs a broad catch, a generic `@throws` tag, or an `ignoreErrors` entry.
