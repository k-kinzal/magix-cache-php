...
`#[CacheComment('...')]` is analysis-only text on a class or method, including an uncached entry point. A method comment replaces the class default; an empty string hides it, and parent-class comments are never inherited implicitly. Comments stay on their own analysis nodes and appear in tree, JSON, and Mermaid output. They never enter runtime declarations, key fingerprints, cache metadata, or analysis problems. Read literals without executing code and visibly mark unresolved expressions; escape comment text for the output format.
