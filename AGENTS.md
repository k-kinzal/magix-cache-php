...
`analyze --uncached=between|all|none` filters ordinary rows after analysis, with `between` as the default: retain only ordinary descendants with both a cached ancestor and a cached descendant. `all` includes wholly uncached branches; `none` omits ordinary rows and promotes visible cached descendants. The explicitly selected root is retained in every mode. `--show-uncached` is a compatibility alias selecting `all`. `--ignore` still prunes the matched node and its entire subtree before promotion. Every renderer preserves the original effects and gap diagnostics, and depth counts original calls rather than visible rows.
