...
use Magix\Cache\Cli\Graph\LocalRestrictions;
...
final class LocalRestrictionsTest extends TestCase
{
    ...
    public static function providerTtlCases(): iterable
    {
        ...
        yield 'equal parent' => [60, null, TtlEstimate::known(60), null];
        yield 'longer parent' => [120, null, TtlEstimate::known(60), null];
        ...
        yield 'unknown dependency' => [20, null, TtlEstimate::unknown(), null];
        yield 'unknown with upper bound' => [20, null, TtlEstimate::unknown(60), null];
        ...
        yield 'uncertain runtime condition' => [20, null, TtlEstimate::unknown(60, 'runtime condition', 30, true), null];
    }

    #[DataProvider('providerTtlCases')]
    public function testTtlOnlyReportsProvenPolicyCaps(int|Ttl $ttl, ?int $maxTtl, TtlEstimate $upstream, ?string $reason): void
    {
        ...
        self::assertSame($reason, $effect->localRestrictions['ttl'] ?? null);

    }

    public function testLeafDeclarationsAndInheritedRestrictionsAreNotStops(): void
    {
        ...
        self::assertSame([], $leaf->localRestrictions);
        self::assertSame([], $calculator->calculate($boundary, new DependencyConstraint(), new StrategyEffect('S::create()', TtlEstimate::unconstrained()))->localRestrictions);
        self::assertSame([], $parent->localRestrictions);
        self::assertSame([], $entry->localRestrictions);
    }

    public function testACloserStrategyConstraintPreventsFalsePolicyAttribution(): void
    {
        $boundary = new BoundaryDeclaration('Page', 'execute', 'a.php', 1, new PolicyDeclaration(PolicySource::MethodAttribute, 20));
        $strategy = new StrategyEffect('S::create()', TtlEstimate::known(10), addsConstraint: true);
        ...
        self::assertSame([], $effect->localRestrictions);
    }

    public function testPolicyCapsOnComposedStrategiesAreVisibleWithoutBoundaryChildren(): void
    {
        $boundary = new BoundaryDeclaration('Page', 'execute', 'a.php', 1, new PolicyDeclaration(PolicySource::MethodAttribute, 20));
        $strategy = new StrategyEffect('S::create()', TtlEstimate::known(60), addsConstraint: true);
        $effect = (new EffectCalculator())->calculate($boundary, new DependencyConstraint(), $strategy);

        self::assertSame(['ttl' => 'local ttl 20s; composed 60s'], $effect->localRestrictions);
    }
    ...
    public function testRuntimeTtlChoicesAreNotAttributedToThePolicy(): void
    {
        ...
        self::assertSame([], $calculator->calculate($dynamic, $constraint)->localRestrictions);
        self::assertSame([], $calculator->calculate($parameter, $constraint)->localRestrictions);
    }
    ...
    public function testDescribeLocalVisibilityAndScopeRestrictSharedDependencies(): void
    {
        ...
        self::assertSame(['visibility' => 'declared by the policy; composed shared'], $calculator->calculate($private, $constraint)->localRestrictions);
        self::assertSame(['visibility' => 'restricted by a scoped parameter; composed shared'], $calculator->calculate($scoped, $constraint)->localRestrictions);
        self::assertSame(['visibility' => 'declared by the policy; composed shared'], $calculator->calculate($noStore, $constraint)->localRestrictions);
    }

    public function testUncertainVisibilityAndInvalidParametersDoNotClaimAStop(): void
    {
        $calculator = new EffectCalculator();
        ...
        self::assertSame([], $calculator->calculate($boundary, $uncertain)->localRestrictions);
        self::assertSame([], $calculator->calculate($invalid, new DependencyConstraint(TtlEstimate::known(60), hasDependencies: true))->localRestrictions);
    }
}
