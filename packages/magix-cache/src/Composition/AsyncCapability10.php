...
/**
 * Holds ten typed cached values until they are mapped to a result.
 *
 ...
 * @template-covariant T10
 */
final readonly class AsyncCapability10
{
    /**
    ...
    public function __construct(
        ...
        private AsyncCached $ninth,
        private AsyncCached $tenth,
    ) {
    }
     ...
     *
     * @template R
     * @param Closure(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10): R $transform
     * @return AsyncCached<R>
     */
    public function map(Closure $transform): AsyncCached
    {
        return AsyncCached::sequence([$this->first, $this->second, $this->third, $this->fourth, $this->fifth, $this->sixth, $this->seventh, $this->eighth, $this->ninth, $this->tenth])->map(static function (array $values) use ($transform): mixed {
            /** @var array{T1, T2, T3, T4, T5, T6, T7, T8, T9, T10} $values */
